...
st.write('''
         ...
         Please fill in the attributes below 
...
def get_dataset():
    data = pd.read_csv(r'C:\Users\KAMALINI\Desktop\finalyearprj\countries of the world.csv')
    return data
...
option=st.sidebar.selectbox('Which algorithm do you want to use?',('Gradient Boosting','Random Forest','Linear Regression'))
if option=='Gradient Boosting':  
    ...
    model=RandomForestRegressor(n_estimators=100,max_depth=6,min_weight_fraction_leaf=0.05,max_features=0.8,random_state=42)
    model.fit(X_train,y_train)
    rf_predictions = model.predict(user_input)
    st.write('The estimated GDP per capita is: ', rf_predictions)
elif option=='Linear Regression':
    model1=LinearRegression()
    model1.fit(X_train,y_train)
    lr_predictions = model1.predict(user_input)
    st.write('The estimated GDP per capita is: ', lr_predictions)
